...
function [gndf,gndm]= NE_backward(pos,ori,alpha,beta,COM,mass)
    ...
    for i=12:-2:6
        for j=1:length(pos)-5
            ...
            if i==12
                fr(i,:,j)= alpha(i,:,j) -  mass(i)*g ;
                mr(i,:,j)= beta(i,:,j) + cross(R*COM(i,:)',mass(i)*g);
            else
                fr(i,:,j)= alpha(i,:,j) -  mass(i)*g + fr(i+2,:,j);
                mr(i,:,j)= beta(i,:,j) + cross(R*COM(i,:)',mass(i)*g)...
    ...
    for i=3:-1:1
        for j=1:length(pos)-5
            R= rotx(ori(i,1,j))*roty(ori(i,2,j))*rotz(ori(i,3,j));
            if i==3
                fr(i,:,j)=alpha(i,:,j)-mass(i)*g + fr(4,:,j) + fr(6,:,j) +fr(5,:,j);
                ...
                fr(i,:,j)=alpha(i,:,j)-(mass(i))*g + fr(i+1,:,j);
                mr(i,:,j)=beta(i,:,j)+ cross(R*COM(i,:)',(mass(i))*g)...
    ...
    gndf=fr(20,:,:)+fr(19,:,:);
    gndf=reshape(gndf,[3,length(pos)-5]);
    gndm=mr(20,:,:)+mr(19,:,:);
    gndm=reshape(gndm,[3,length(pos)-5]);
    gndm = -gndm;
